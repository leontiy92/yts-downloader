...
class Vars:
  def __init__(self, aria2 = None, movies = None, MAX_CON = 0, MOVIE_QUALITY = "", DL_DIR = "", TEMP_DIR = ""):
    self.aria2 = aria2
    self.movies = movies
...
def scan_aria():
  ...
  logging.debug(f"aria queue: {num_dls}: {len(complete)} complete, {len(active)} active")
...
def run():
  while True:
    ...
    # TODO: handle case we downloaded all movies
    for m in v.movies.aggregate([{"$match": "files": {"$exists": False}}}, {"$sample": {"size": 1}}]):
      _id = m["_id"]
